...
def build_ivfpq_index(embeddings, nlist=100, m=8):
    dim = embeddings.shape[1]
...
if __name__ == "__main__":
    ...
    ivfpq_index = build_ivfpq_index(embeddings)

    pred_distances, pred_indices = ivfpq_index.search(query_embeddings.astype(np.float32), k)

    recall = compute_recall(pred_indices, gt_indices)

    print("IVF-PQ + Random-Walk Recall@{}: {:.4f}".format(k, recall))
